    ...
    def generate(
        ...
        **kwargs
        ...
        for block_idx in range(num_blocks):
            ...
            x_init = mask_id * torch.ones((input_ids.shape[0], block_size-prompt_length%block_size), device=input_ids.device, dtype=torch.long)
            x_init = torch.cat([input_ids, x_init], dim=1)

            x_t = x_init.clone()
            block_past_key_values = None
            
            print(f"DEBUG: Starting block {block_idx}/{num_blocks}. max_new_tokens={max_new_tokens}")
            ...
            while True:
                ...
                # print(f"DEBUG: mask_sum={mask_idx.sum()}")
